(ns jobsearchops.operation
  "OperationActor -- one job-search-portal operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (JobSearch-LLM) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the Job Search Portal Governor (:govern)
  ...
  One graph run = one job-search-portal operation (ingest -> advise ->
  ...
  decision to a human operator (portal operator/content coordinator).
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:posting/publish`/`:posting/delist` ALWAYS reach this
  node when the governor is clean -- see `jobsearchops.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [jobsearchops.jobsearchopsllm :as jobsearchopsllm]
...
(defn build
  ...
  `jobsearchops.store/Store`).
  opts:
    :advisor      -- a `jobsearchops.jobsearchopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (jobsearchopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; JobSearch-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (jobsearchopsllm/-advise advisor store request)]
            {:proposal p :audit [(jobsearchopsllm/trace request p)]})))

      ;; Job Search Portal Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
